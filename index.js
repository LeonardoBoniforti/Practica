...
contenedorBotones.addEventListener('click', (e) => {
    ...
    if(tipo === 'exito') {
        console.log('Agregando toast de exito');
        agregarToast({tipo: 'exito', titulo: 'Exito!', descripcion: 'La operación fue exitosa.', autoCierre: true})
    }
    if(tipo === 'error') {
        console.log('Agregando toast de error');
        agregarToast({tipo: 'error', titulo: 'Error!', descripcion: 'Hubo un error.', autoCierre: true})
    }
    if(tipo === 'info') {
        console.log('Agregando toast de info');
        agregarToast({tipo: 'info', titulo: 'Info!', descripcion: 'Esta es una notificción de información.'})
    }
    if(tipo === 'warning') {
        console.log('Agregando toast de warning');
        agregarToast({tipo: 'warning', titulo: 'Warning!', descripcion: 'Ten cuidado.'})
    }
});

// Even Listener para detectar click en los toast.
...
//Función para agregar la clase cerrando al toast.
const agregarToast = ({tipo, titulo, descripcion, autoCierre}) => {
    ...
    // Agreagamos Event Listener para detectar cuando termine la animacion
